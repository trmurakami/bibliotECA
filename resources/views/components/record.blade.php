<div class="card mb-3">
    <div class="card-header">
        <small class="text-body-secondary">{{ $work->type }}</small>
    </div>
    <div class="row g-0">
        <div class="col-lg-2 d-flex align-items-center justify-content-center">
            @if ($work->cover)
            <a href="{{ route('works.show',$work->id) }}">
                <img src="{{ asset('storage/cover/'.$work->cover) }}" class="img-fluid rounded-start" alt="Cover">
            </a>
            @else
            @if ($work->type == 'Livro')
            <a href="{{ route('works.show',$work->id) }}">
                <img src="data:image/png;base64,"
                    class="mx-auto d-block" alt="Cover">
            </a>
            @elseif ($work->type == 'Artigo')
            <a href="{{ route('works.show',$work->id) }}">
                <img src="data:image/png;base64,"
                    class="mx-auto d-block" alt="Cover">
            </a>
            @elseif ($work->type == 'Trabalho em Evento')
            <a href="{{ route('works.show',$work->id) }}">
                <img src="data:image/png;base64,"
                    class="mx-auto d-block" alt="Cover">
            </a>
            @endif


            @endif
        </div>
        <div class="col-lg-10">
            <div class="card-body">
                <h5 class="card-title"><a href="{{ route('works.show',$work->id) }}">{{ $work->name }}
                        ({{ $work->datePublished }})</a></h5>
                @if ($work->doi)
                <p class="card-text"><small class="text-body-secondary">DOI:
                        <a href="https://doi.org/{{ $work->doi }}" target="_blank">{{ $work->doi }}</a>
                    </small></p>
                @endif

                <p class="card-text"><small class="text-body-secondary">Autores:
                        <ul>
                            @foreach ($work->authors as $author)
                            <li class="list-group-item">
                                <a href="/works?author={{ $author->name }}">{{ $author->name }}</a>
                                @if ($author->id_lattes13)
                                <a href="https://lattes.cnpq.br/{{ $author->id_lattes13 }}" target="_blank">
                                    <img src="data:image/png;base64,"
                                        height="15" />
                                </a>
                                @endif
                            </li>
                            @endforeach
                        </ul>
                    </small>
                </p>
                @if (isset($work->isPartOf_name))
                <p class="card-text">
                    <small class="text-body-secondary">
                        Publicado em: {{ $work->isPartOf_name }}
                        @if (isset($work->issn))
                        (ISSN: {{ $work->issn }})
                        @endif
                        @if (!empty($work->volumeNumber))
                        Vol.: {{ $work->volumeNumber }}
                        @endif
                        @if (!empty($work->issueNumber))
                        , Num.: {{ $work->issueNumber }}
                        @endif
                        @if (!empty($work->pageStart))
                        , p. {{ $work->pageStart }} - {{ $work->pageEnd }}
                        @endif
                    </small>
                </p>
                @endif
                @if (isset($work->releasedEvent))
                <p class="card-text">
                    <small class="text-body-secondary">
                        Apresentado no evento: {{ $work->releasedEvent }}
                        @if (!empty($work->volumeNumber))
                        Vol.: {{ $work->volumeNumber }}
                        @endif
                        @if (!empty($work->issueNumber))
                        , Num.: {{ $work->issueNumber }}
                        @endif
                        @if (!empty($work->pageStart))
                        , p. {{ $work->pageStart }} - {{ $work->pageEnd }}
                        @endif
                    </small>
                    @endif
                <p class="card-text">{{ $work->description }}</p>
            </div>
        </div>
    </div>
</div>